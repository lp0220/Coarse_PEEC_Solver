...
{
	// Try direct path first (integrated mode), fallback to legacy subfolder
	std::string l_file = File_Path + "L.txt";
	std::string p_file = File_Path + "P.txt";
	std::string b2n_file = File_Path + "B2N.txt";
	std::string port_file = File_Path + "PORT.txt";
	{
		std::ifstream test(l_file);
		if (!test.is_open()) {
			l_file = File_Path + "746/LL_OO.txt";
			p_file = File_Path + "746/PP_OO.txt";
			b2n_file = File_Path + "746/B2N.txt";
			port_file = File_Path + "746/PORT.txt";
		}
	}
	if (!Read_Matrix_From_File(l_file, LL)) return false;
	if (!Read_Matrix_From_File(p_file, PP)) return false;
	if (!Read_Matrix_From_File(b2n_file, CN)) return false;
	std::ifstream fin(port_file);
	if (!fin.is_open()) {
		std::cerr << "无法打开文件: " << port_file << std::endl;
		return false;
	}
